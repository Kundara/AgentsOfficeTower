...
- Changed hot-file icons to familiar desktop file semantics: scripts and documents use folded pages with restrained subtype labels, while images, vector artwork, audio, video, tables, databases, archives, fonts, settings, Unity assets, shaders, animation, and binaries use dedicated pictograms. Text-bearing badges now enforce at least 4.5:1 label contrast, including white Markdown lettering on a dark-blue label. Change state remains a separate compact corner badge.
...
- Fixed abandoned Codex turns remaining counted as live and occupying Party Game desks for days when `turn_aborted`, `idle`, or `notLoaded` arrived without a `final_answer`. Active status remains authoritative and fresh quiet work keeps its short bridge, but dormant ongoing latches, subscriptions, and paginated-list fallbacks now expire after the existing three-minute quiet-work window.
- Fixed Claude Code sessions and inferred background-task children appearing multiple times on a merged repository floor when Agent SDK worktree-inclusive reads returned the same logical session for the main checkout, sibling worktrees, and nested project roots. Project snapshots now enforce canonical session ownership, while the browser defensively collapses repeated logical IDs and prefers the snapshot matching the agent's real `cwd`.
...
- Added a trustworthy-visibility plane: adapter health now survives snapshot assembly as `DashboardSnapshot.providerHealth` (with a distinct `unconfigured` status for intentionally absent integrations), `/api/health/live`, `/api/health/ready`, and `/api/health` report fleet-level healthy/starting/degraded/stale with build identity and a fleet pulse, and the CLI gained `aot status`, `aot doctor`, `aot digest`, and `--version`.
- Added browser trust surfaces: a hero coverage chip opening a fleet coverage drawer (per-provider status, stale floors, what-might-be-missing notes, notification toggle), degraded/stale floor chips, session stale and time-in-state badges, and a per-session `Why?` evidence view showing the seating signal, typed vs inferred provenance, freshness ages, the retention rule in effect, and the recent typed event chain.
- Added Sessions triage: a search box and lens selector (needs intervention, my live work, inferred only, remote work, possible overlap, degraded visibility), oldest-wait-first Needs You ranking, plus evidence-backed possible-overlap cards derived from multi-actor hot changes with a Show-sessions action, and opt-in desktop notifications with a ten-minute stale-wait escalation.
...
- Changed projectless Hermes roaming agents to hover in a fixed left-side sky layer outside the building, with non-colliding screen-space placement and fly-in/fly-out handoffs when the same Hermes session gains or loses a project floor relation.
- Changed Hermes same-session project-floor moves to use a short fixed-layer transfer ghost between old and new desk hit rects, so cross-floor reassignment reads as one moving orchestrator instead of two disconnected scene changes.
- Changed Hermes hook project matching so a session keeps its last project through 20 rootless hook actions, then becomes projectless/roaming if no known project root appears again.
- Changed OpenClaw workspace matching to seat configured child workspaces under known project roots and render unmatched active Gateway sessions as `openclaw:roaming` orchestrators in the same fixed left-side sky layer and screen-space handoff path as projectless Hermes.
...
- Fixed shared-room and local floor grouping to match normalized repository URLs first and unambiguous root-commit fingerprints second, so peers on the same repository no longer split into differently named floors when their histories report different root commits or renamed remote URLs.
...
- Fixed settled idle Codex subagents so child workers without a final-answer phase still release after completion instead of remaining `isOngoing` and inflating active totals.
...
- Fixed avatar rebuild handoffs so replacement Pixi sprites inherit the active motion position before the offscreen scene root is swapped in, avoiding one-frame destination snaps during refreshes.
...
- Updated README, architecture, product spec, integration hooks, and self-development guidance for shared Hermes/OpenClaw roaming-orchestrator motion.
...
- Updated README, architecture notes, integration hooks, self-development checks, and references for Claude Code dynamic workflows / `ultracode`, Agent View background jobs, and the OpenTelemetry protocol path needed for future hookless workflow-subagent visibility.
- Updated README, product spec, integration hooks, self-development checks, and references to document local Claude workflow/subagent transcript and journal discovery, including hook-backed confidence upgrades.
