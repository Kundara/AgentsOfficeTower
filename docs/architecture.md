...
- Browser mode
  ...
  - the lowest all-workspaces level is a synthetic `street-cafe` scene: canonical projectless Codex roots and locally materialized Claude Home work agents are removed from duplicate workspace floors, cloned with their real `sourceProjectRoot` / `sourceAgentId`, and combined at café-table workstations; an explicit future `interactionMode: "work"` can join the same floor without guessing from broad transport labels such as `vscode`, CLI, exec, or app-server-owned work
