...
1. `codex app-server`
   ...
   In this codebase, that still means we need a runnable Codex executable. We honor `CODEX_CLI_PATH` first; on macOS we then prefer the Codex runtime bundled with `ChatGPT.app` or `Codex.app` before `codex` on `PATH`. On native Windows we can extract the Store app's packaged `codex.exe` into a local cache before PATH and WSL fallbacks. Tower launches its read-only observer with empty `mcp_servers` and `plugins` overrides because thread observation does not need the user's configured tool processes; interactive Codex sessions keep their normal configuration.
...
- recent file-change events also feed a derived workspace `activity` block on each `DashboardSnapshot`; that block powers the in-scene Ops Wall with decayed, family-balanced file scores plus format/color/change-kind metadata while tool and command activity stay out of the hot-stuff board
...
- when a resumed/subscribed desktop thread hydrates to a completed or interrupted latest turn that still lacks a `final_answer`, the live monitor preserves it through the three-minute quiet-work window instead of dropping it between progress messages; an `idle`/`notLoaded` latch older than that window expires
...
- observer runtime unload notifications such as `thread/closed` or `thread/status/changed -> notLoaded` are treated as subscription state, not immediate proof that the underlying thread resolved; successful rereads preserve fresh non-final work but release dormant occupancy after the quiet-work window or a final answer
- non-final `turn/completed` and `turn/interrupted` events are treated as update boundaries, not immediate stop signals, so desktop sessions stay desk-owned between assistant progress messages while fresh; abandoned dormant turns cannot retain that ownership indefinitely
- quiet desk-live local work can remain current for about 3 minutes when it has recent non-final activity, is still on a live subscription, or is sitting in a transient `notLoaded` transport state; typed `active` status remains authoritative, while `idle`/`notLoaded` ongoing latches use the same bounded window
...
- fleet mode only keeps workspaces whose normalized agent/session-log timestamps are no older than 7 days; launch seeds and config-only roots are path aliases rather than unconditional floors, while explicit project mode remains pinned
  ...
  - Git-linked worktrees merge onto one repo floor by default, with a global split toggle available when one floor per worktree is more useful. The merged roster deduplicates stable logical agent IDs across project snapshots and prefers the source snapshot whose root matches the agent's `cwd`, preventing worktree-inclusive provider reads from multiplying one session across the floor
...
- an in-scene Ops Wall sits on the primary room wall between the left edge and door, using a compact title-free 3x3 grid ranked across detailed file families; each row carries a format-colored SVG family mark plus an added/modified/deleted/renamed/mixed badge without adding a separate admin dashboard surface
- local threads remain seated while the thread is still actively ongoing, and fresh dormant transitions bridge short pauses between visible events without turning abandoned work into permanent occupancy
...
- transient `status.type = notLoaded` unloads reread before release; a final answer clears immediately, while a dormant non-final latch clears after the three-minute quiet-work window
- recent non-final, subscribed, or transiently `notLoaded` desk-live locals keep their workstation for about 3 minutes after the last update; only authoritative `active` status remains exempt from that dormant-age cap
...
- the primary room wall includes a compact scene-native hot-stuff board between the left wall and doorway, built from decayed `activity.hotChanges`; entries retain a backward-compatible script/doc/media type plus specific family, format, brand color, and change kind. Snapshot assembly supplements raw events from the agent's typed latest-file activity and a host-native Git status fallback that includes tracked and untracked files, while shared-room merges deduplicate matching paths and reapply the same three-per-family cap. The CLI `gist` command exposes that same hot-change summary alongside active-agent last-message and last-file-change hints for light state sync before deeper reads
...
- file-format audit route
  `/file-format-audit` runs the production hot-file SVG renderer against the complete core format catalog, including semantic pictogram review plus interactive change-state, scale, surface, and search controls before office-map changes ship
...
- browser placement rules are intentionally a little stickier than raw workload freshness, because a live local thread should not visually bounce desk -> rec -> desk during short polling gaps; that stickiness is bounded for dormant `idle`/`notLoaded` rows
...
- once `isOngoing` is present on a local thread, the browser keeps that thread classified as busy across workstation and session-list logic while the monitor still validates the signal; stale dormant signals are cleared before they reach either surface
...
- when the Anthropic Agent SDK is available, Claude Code project discovery prefers `listSessions()` and per-session `cwd` metadata before falling back to raw directory scanning; project snapshots retain only sessions canonically owned by that exact project root even when the SDK returns worktree-inclusive results, and this inventory does not represent Claude Home account chats
...
- Claude `Bash` calls with `run_in_background` create distinct inferred `claude:background-task` children under the lead session; an incremental, bounded-LRU raw-transcript scan preserves task notifications omitted by the SDK window, while bounded temporary-output tails provide the latest visible output and file mtime activity
- background-task completion, failure, and stop records clear ongoing state explicitly; unfinished records with no fresh output expire after 30 minutes, completed rows retain only the normal short recent-work grace, and at most 24 task rows are retained per session before output tails are opened
- project-owned Claude rows can open the generic read-only in-scene history card, including background-task activity/output, while account-observed Claude Home rows remain non-interactive and browser reply controls remain Codex-only
- raw Claude background-command output is local-only: shared-room payload construction removes the child row's output-derived detail and latest message before transmission
