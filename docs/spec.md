...
- Personal Free, Pro, and Max Claude Home Recent chats have no supported live local listing API. They must not be inferred from `claude-code-sessions`, private renderer endpoints, cookies, or browser cache. Enterprise Compliance chat metadata is an explicit future opt-in administrative provider, not part of the local adapter, and it does not expose Home/Cowork work activity.
...
- A normal repository merely named `Chat` must not enter Chat Café. ChatGPT account sidebar history and personal Claude Home Recent chats must not be synthesized when their supported local APIs do not expose them.
