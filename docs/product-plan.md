...
- [ ] **Publish the CLI to npm** (initial `0.1.x`), with `npx agents-office-tower demo preview` as the documented zero-install pitch path. **Ready:** packages have `files` allowlists, publishable version-range deps, and a tag-driven `.github/workflows/release.yml` (core → web → cli, with provenance). **Blocked on one user action:** the `agents-tower` npm org exists (created 2026-07-11) and CI is green on master (full suite passes on ubuntu) — add an `NPM_TOKEN` repo secret, then push a `v0.1.x` tag.
...
2. ~~npm scope~~ Fully resolved 2026-07-11: the `agents-tower` org exists, v0.1.0 and v0.1.1 are published, all three packages have a trusted publisher (`Kundara/AgentsOfficeTower` / `release.yml`, npm publish permission) and the strictest publishing access (2FA required, tokens disallowed). Tagged releases publish tokenlessly with provenance; no `NPM_TOKEN` secret exists or is needed.
