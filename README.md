...
- finished subagents keep a longer visible desk cooldown, then walk back out through the room door instead of blinking away
...
- runtime-active local subagents stay visible even when `isCurrent` briefly shifts to a sibling update or the parent thread
...
- scene/session labels should normalize repo-local paths so raw WSL mount paths do not leak into hover cards or session titles
